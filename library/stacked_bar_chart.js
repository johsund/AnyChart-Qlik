...
// create bar chart
window['chart' + layout.qInfo.qId] = anychart.bar();
...
var tooltipSettings = window['chart' + layout.qInfo.qId].tooltip();
...
for(index = 1; index<seriesData.length;index++) {
	  ...
	  series = window['chart' + layout.qInfo.qId].bar(seriesData[index]);
...
window['chart' + layout.qInfo.qId].listen("pointClick", function(e) {
	...
	if (document.getElementById(canSelName)) {
		...
		$("#" + canSelName).click(function(){
			...
			//app.field(layout.qHyperCube.qDimensionInfo[1].qGroupFieldDefs[0]).selectValues([], false, false);
			qlik.resize();
			
			//window['viz' + layout.qInfo.qId]($element, layout, self, app);
			
			// for(i=0;i<=window['chart' + layout.qInfo.qId].bar.length;i++) {
