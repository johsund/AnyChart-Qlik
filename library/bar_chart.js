...
//$("#container_" + layout.qInfo.qId).css("overflow-y", "scroll");
...
if(layout.datascroll) {
	...
	if(parseInt(layout.scrollaftermax)<=data.length) {	
		...
		// // autoHide the scroller
		 window['chart' + layout.qInfo.qId].xScroller().autoHide(true); //works
...
var series = window['chart' + layout.qInfo.qId].bar(dataSet);
...
//Labels 
if(layout.dataonpoints) {series.labels().enabled(true);series.labels().position('right')};
